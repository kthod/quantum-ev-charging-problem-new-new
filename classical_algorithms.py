...
def optimize_charging_schedule(e ,d ,energy_limit,T,DT):
    ...
    return evs_charging_schedules


import numpy as np

def optimize_charging_schedule_llf(epsilon, d, energy_limit, T, DT):
    e = epsilon.copy()
    voltage = 240  # Constant voltage
    currents = [0, 16, 32, 48]  # Available currents
    evs_charging_schedules = np.zeros((len(epsilon), T))
    energy_limits = np.array(T * [energy_limit])
    ...
    for timestep in range(T):
        ...
        laxities = []
        for ev in range(len(epsilon)):
            laxity = calculate_laxity(epsilon[ev], d[ev] - timestep, currents[-1])
        ...
        for laxity, ev in sorted_evs_by_laxity:
            remaining_energy_demand = epsilon[ev]
...
def optimize_charging_schedule_edf(epsilon, d, energy_limit, T, DT):
    e= epsilon.copy()
    voltage = 240  # Constant voltage
    currents = [0, 16, 32, 48]  # Available currents
    evs_charging_schedules = np.zeros((len(e), T))
